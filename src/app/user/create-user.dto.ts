...
import { IsArray, IsBoolean, IsOptional, IsString, IsIn, IsNotEmpty } from 'class-validator';
...
export class CreateUserDto {
    ...
    @ApiProperty({ example: '123e4567-e89b-12d3-a456-426614174090', description: 'User country ID (UUID)', required: false })
    @IsString()
    @IsOptional()
    countryId: string;
    ...
    @ApiProperty({ example: 'EMAIL', description: 'Authentication method (EMAIL or PHONE)' })
    @IsString()
    @IsNotEmpty()
    @IsIn(['EMAIL', 'PHONE'])
    authMethod: string;
