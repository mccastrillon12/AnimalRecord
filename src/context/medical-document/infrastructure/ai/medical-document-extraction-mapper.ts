...
import {
  ...
  ExtractedDiagnosis,
  ExtractedLaboratoryReport,
  ExtractedLaboratoryResult,
...
export class MedicalDocumentExtractionMapper {
  map(customOutput?: string, standardOutput?: string): MedicalDocumentAnalysis {
    ...
    let extraction: MedicalDocumentExtraction = {
      ...
      laboratoryReport: this.mapLaboratoryReport(inference, explainability),
      laboratoryResults: this.mapLaboratoryResults(
        this.arrayValue(inference, [
          'laboratory_results',
          'laboratoryResults',
          'lab_results',
  ...
  private extractionForCategory(
    ...
    if (category === MedicalDocumentType.LaboratoryResult) {
      return {
        ...common,
        laboratoryReport: extraction.laboratoryReport,
        laboratoryResults: extraction.laboratoryResults,
        // Preserve only the closed transcription contract. Unexpected fields
        // could contain comparisons or clinical conclusions generated by AI.
        additionalFields: {},
      };
    }
    return common;
  ...
  private mergeExtraction(
    ...
    return {
      ...
      laboratoryReport:
        current.laboratoryReport || next.laboratoryReport
          ? this.mergeLaboratoryReports(
              current.laboratoryReport,
              next.laboratoryReport,
            )
          : undefined,
      laboratoryResults: [
        ...(current.laboratoryResults || []),
        ...(next.laboratoryResults || []),
      ],
      referral:
  ...
  private ensureUniqueItemIds(
    ...
    return {
      ...
      laboratoryResults: normalize(extraction.laboratoryResults || []),
    };
  }
  ...
  private mapLaboratoryReport(
  ...
  ): ExtractedLaboratoryReport | undefined {
    const report = this.asObject(
    ...
    return {
      reportNumber: this.stringValue(report, [
        'report_number',
        'reportNumber',
        'numero_reporte',
      ]),
      orderNumber: this.stringValue(report, [
        'order_number',
        'orderNumber',
        'numero_orden',
      ]),
      specimenType: this.stringValue(report, [
        'specimen_type',
        'specimenType',
        'sample_type',
        'tipo_muestra',
      ]),
      specimenStatus: this.stringValue(report, [
        'specimen_status',
        'specimenStatus',
        'sample_status',
        'estado_muestra',
      ]),
      collectionDate: this.stringValue(report, [
        'collection_date',
        'collectionDate',
        'fecha_toma',
      ]),
      receivedDate: this.stringValue(report, [
        'received_date',
        'receivedDate',
        'fecha_ingreso',
      ]),
      reportDate: this.stringValue(report, [
        'report_date',
        'reportDate',
        'fecha_reporte',
        'fecha_emision',
      ]),
      analysisDate: this.stringValue(report, [
        'analysis_date',
        'analysisDate',
        'fecha_analisis',
      ]),
      methods: this.stringArrayValue(report, ['methods', 'metodos']),
      equipment: this.stringArrayValue(report, ['equipment', 'equipos']),
      analysts: this.stringArrayValue(report, ['analysts', 'analistas']),
      reviewers: this.stringArrayValue(report, ['reviewers', 'revisores']),
      reportedComments: this.stringArrayValue(report, [
        'reported_comments',
        'reportedComments',
        'laboratory_comments',
        'observaciones_reportadas',
      ]),
      confidence: this.itemConfidence(report, evidence),
      source: this.mapSource(report) || this.mapEvidenceSource(evidence),
    };
  }

  private mapLaboratoryResults(
    values: unknown[],
    evidenceValues: unknown[] = [],
  ): ExtractedLaboratoryResult[] {
    return values
      .map((value, index) => {
        const item = this.asObject(value);
        const evidence = this.asObject(evidenceValues[index]);
        const flag = this.stringValue(item, [
          'flag',
          'marker',
          'marcador',
          'bandera',
        ]);
        const result = this.laboratoryResultWithoutDuplicatedFlag(
          this.stringValue(item, ['result', 'resultado', 'value', 'valor']),
          flag,
        );
        return {
          id: this.itemId(item, 'laboratory-result', index),
          name: this.itemName(value, item, [
            'name',
            'test_name',
            'testName',
          ...
          result,
          unit: this.stringValue(item, ['unit', 'unidad']),
          referenceRange: this.stringValue(item, [
            'reference_range',
            'referenceRange',
            'reference_value',
            'rango_referencia',
          ...
          flag,
          method: this.stringValue(item, ['method', 'metodo']),
          technicalDetails: this.stringValue(item, [
            'technical_details',
            'technicalDetails',
            'measurement_details',
            'detalles_tecnicos',
  ...
  private laboratoryResultWithoutDuplicatedFlag(
    result?: string,
    flag?: string,
  ): string | undefined {
    if (!result || !flag || !['*', 'H', 'L'].includes(flag)) return result;

    if (result.startsWith(flag)) {
      return result.slice(flag.length).trim() || result;
    }
    if (result.endsWith(flag)) {
      return result.slice(0, -flag.length).trim() || result;
    }
    return result;
  }

  private mergeLaboratoryReports(
    current?: ExtractedLaboratoryReport,
    next?: ExtractedLaboratoryReport,
  ): ExtractedLaboratoryReport {
    return {
      ...next,
      ...current,
      methods: this.uniqueStrings([
        ...(current?.methods || []),
        ...(next?.methods || []),
      ]),
      equipment: this.uniqueStrings([
        ...(current?.equipment || []),
        ...(next?.equipment || []),
      ]),
      analysts: this.uniqueStrings([
        ...(current?.analysts || []),
        ...(next?.analysts || []),
      ]),
      reviewers: this.uniqueStrings([
        ...(current?.reviewers || []),
        ...(next?.reviewers || []),
      ]),
      reportedComments: this.uniqueStrings([
        ...(current?.reportedComments || []),
  ...
  private toDocumentType(value?: string): MedicalDocumentType {
    ...
    if (
      normalized.includes('laboratory_result') ||
      normalized.includes('laboratory_report') ||
      normalized.includes('lab_result') ||
      normalized.includes('resultado_laboratorio') ||
      normalized.includes('informe_laboratorio') ||
      normalized.includes('hemograma') ||
      normalized.includes('hematologia') ||
      normalized.includes('blood_chemistry') ||
      normalized.includes('quimica_sanguinea') ||
      normalized.includes('coproparasitologico') ||
      normalized.includes('q_pcr')
    ) {
      return MedicalDocumentType.LaboratoryResult;
    }
    if (
